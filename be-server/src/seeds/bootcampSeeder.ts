...
// Helper buat tanggal relatif dari sekarang (dalam hari)
      ...
      mentors: [
        {
          ...
          occupation: 'Data Scientist @ Bukalapak',
        },
      ],
    },
    batches: [
...
async function seed() {
  try {
    ...
    // Bersihkan data lama agar seeder idempotent
    await Promise.all([
    ...
    for (const data of seedData) {
      ...
      for (const batchData of data.batches) {
        ...
        for (const s of sessionData) {
          const sessionDate = daysFromNow(s.daysOffset);
